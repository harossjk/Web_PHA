...
interface props {
    setInquiryData: any;
    moldName: any;
    onChangeInquiry: (data: any, chName: string, reportExport?: boolean) => void;
}

const Filter = ({ setInquiryData, moldName, onChangeInquiry }: props) => {

    const { MSMDStore, ProcessAbilityStore } = useStore();
    ...
    const handleInquiry = (filterData: any, reportExport: boolean = false) => {
        ...
        //export
        if (reportExport) {
            onChangeInquiry(filterData, 'P1', reportExport)
        }

        setInquiryData(filterData)
        onChangeInquiry(filterData, 'P1')
